...
class GFFormsModel {
			...
			if( $is_entry_update || ! GFFormsModel::is_field_hidden( $form, $field, array(), $read_value_from_post ? null : $lead ) ) {
                ...
                if ( is_array( $inputs ) ) {
                    foreach ( $inputs as $input ) {
                        if($input['id'] != 6.1) {
                            self::check_input( $form, $field, $lead, $current_fields, $input['id'] );
                        }
                    }
                } else {
                    if($inputs->id != 6.1) {
                        self::check_input($form, $field, $lead, $current_fields, $field->id);
                    }
                }
				...
				if ( is_array( $inputs ) ) {
					foreach ( $inputs as $input ) {
					    //if($input['id'] == 1.3) {print_r($input); exit(); } //1
    ...
    public static function check_input( $form, $field, &$lead, $current_fields, $input_id ) {
        global $wpdb;
        ...
        $lead_id_continue = intval($wpdb->get_var($wpdb->prepare( "SELECT lead_id FROM wp_rg_lead_detail WHERE value = %s ORDER BY lead_id DESC", $value )));

        if($lead_id_continue && $input_id != 6.1) {
            $exists_note = $wpdb->get_row($wpdb->prepare( "SELECT id FROM wp_rg_lead WHERE id = %d AND date_created >= DATE_SUB(NOW(), INTERVAL 2 DAY)", $lead_id_continue));
	...
	public static function update_lead_field_value( $form, $lead, $field, $lead_detail_id, $input_id, $value ) {
		...
		if ( ! rgblank( $value ) ) {
            ...
            //$wpdb->$wpdb->get_row("SELECT lead_id FROM $lead_detail_table  WHERE value = $truncated_value AND ");
